...
class DashConfig():
    ...
    def slurp_config_file(self, filename):
        # read meraki.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is meraki.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        default_port = 1998 if (network == 'mainnet') else 2999

        # use default port for network if not specified in meraki.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from meraki.conf
